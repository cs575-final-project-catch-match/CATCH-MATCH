...
int ch = 256;
int match(string pattern, string text, int l, int j,int pLength, int linenumber)
{
    for (j = 0; j < pLength; j++)
    {
        if (pattern[j]!=text[l+j])
            break;
    }
    
    if (j == pLength){
        cout<<"Pattern fully matched at line "<<linenumber<<" at index "<<l<<"\n"<<endl;
    }
    return l;
}
...
void rabinkarpsearch(string pattern){
    ...
    while(getline(stringfile, text)){
        ...
        while (l<=offset){
            ...
            //To check if hash values of pattern and text are same
            ...
            if ( pHash == tHash )
            {
                ...
                match(pattern,text,l,j,pLength,linenumber);
    ...
    while (getline(stringfile,stringline))
    {
        ...
        while(stringindex < stringlinelength){
            ...
            if(patternindex == patternlinelength){
                cout<<"\nPattern fully matched at line number: "<<stringlinecount<<" at index: "<<searchtime-patternlinelength<<endl;
                ...
                if(stringindex < stringlinelength){
                    ...
                    cout<<"\nPattern partly matched\nCharacters matched of pattern '"<<patternline<<"' at line: "<<stringlinecount<<" at index: "<<searchtime-patternlinelength<<"\nPercentage of pattern matched: "<< percentage <<"%"<<endl;
